...
(() => {
  ...
  const pwx = { cache: {}, at: {}, inflight: {} };

  function elevFor(p) {
    const n = `${p.name || ""} ${p.nameJa || ""} ${p.query || ""}`;
    if (/鶴見岳|鶴見山上|纜車|ロープウェイ/.test(n)) return 1375;
    if (/草千里/.test(n)) return 1140;
    if (/吊橋|九重/.test(n)) return 777;
    if (/黑川|黒川/.test(n)) return 700;
    if (/金鱗|由布院/.test(n)) return 435;
    if (/太宰府/.test(n)) return 40;
    if (/福岡|天神|FUK|ORIX/.test(n)) return 10;
    if (/嬉野|大正/.test(n)) return 80;
    if (/香港|HKG/.test(n)) return 10;
    return 80;
  }
  ...
  function placeVisitDate(p) {
    for (const d of trip.days || []) {
      if ((d.blocks || []).some((b) => b.placeId === p.id)) return d.date;
      const st = stay(d.stayId);
      if (st && st.placeId === p.id) return d.date;
    }
    return viewDate();
  }
  function placeHour(p, ymd) {
    const d = day(ymd);
    const b = (d?.blocks || []).find((x) => x.placeId === p.id && x.time);
    return b?.time || "";
  }
  ...
  function parseOm(data, ymd, hm) {
    const i = (data.daily?.time || []).indexOf(ymd);
    if (i < 0) return null;
    const hi = Math.round(data.daily.temperature_2m_max[i]);
    const lo = Math.round(data.daily.temperature_2m_min[i]);
    const windRaw = data.daily.wind_speed_10m_max?.[i] ?? data.daily.wind_speed_10m_mean?.[i];
    const wind = windRaw != null ? Math.round(windRaw) : 0;
    const code = data.daily.weather_code?.[i];
    let at = null;
    if (hm && data.hourly?.time) {
      const want = new Date(`${ymd}T${hm}:00+09:00`).getTime();
      if (!Number.isNaN(want)) {
        let best = -1;
        let bestD = 1e9;
        data.hourly.time.forEach((t, idx) => {
          const d = Math.abs(new Date(t).getTime() - want);
          if (d < bestD) { bestD = d; best = idx; }
        });
        if (best >= 0 && data.hourly.temperature_2m?.[best] != null) {
          at = Math.round(data.hourly.temperature_2m[best]);
        }
      }
    }
    return { hi, lo, wind, code, at, src: "forecast" };
  }
  function srcLabel(src) {
    if (src === "forecast") return "當日預報";
    if (src === "climate") return "十月平年";
    return "海拔估算";
  }
  function fetchPlaceWx(p, ymd, hm) {
    if (p?.lat == null || p?.lng == null || !ymd) return Promise.resolve(null);
    const key = pwxKey(p, ymd);
    const now = Date.now();
    if (pwx.cache[key] && pwx.at[key] && now - pwx.at[key] < 15 * 60 * 1000) {
      return Promise.resolve(pwx.cache[key]);
    }
    if (pwx.inflight[key]) return pwx.inflight[key];
    const elev = elevFor(p);
    if (!pwx.cache[key]) pwx.cache[key] = { ...typicalOct(elev), elev, ymd };
    const run = (async () => {
      const q = `latitude=${p.lat}&longitude=${p.lng}&elevation=${elev}&timezone=${encodeURIComponent("Asia/Tokyo")}`;
      const daily = "temperature_2m_max,temperature_2m_min,weather_code,wind_speed_10m_max";
      const today = fmt.ymd(tzNow("Asia/Tokyo"));
      const inWindow = ymd >= ymdShift(today, -1) && ymd <= ymdShift(today, 15);
      let rec = null;
      try {
        const url = inWindow
          ? `https://api.open-meteo.com/v1/forecast?${q}&daily=${daily}&hourly=temperature_2m,wind_speed_10m&start_date=${ymd}&end_date=${ymd}`
          : `https://climate-api.open-meteo.com/v1/climate?${q}&daily=temperature_2m_max,temperature_2m_min,wind_speed_10m_mean&start_date=${ymd}&end_date=${ymd}&models=EC_Earth3P_HR`;
        const data = await (await fetch(url)).json();
        if (!data.error) rec = parseOm(data, ymd, hm);
        if (rec && !inWindow) rec.src = "climate";
      } catch { /* keep typical */ }
      if (!rec && inWindow) {
        try {
          ...
          if (!data.error) {
            rec = parseOm(data, ymd, hm);
            if (rec) rec.src = "climate";
          }
        } catch { /* keep typical */ }
      }
      if (!rec) rec = typicalOct(elev);
      rec.elev = elev;
      rec.ymd = ymd;
      pwx.cache[key] = rec;
      pwx.at[key] = Date.now();
      delete pwx.inflight[key];
      return rec;
    })();
  ...
  function paintPlaceWx() {
    $$("[data-pwx]").forEach((el) => {
      const p = place(el.dataset.pwx);
      if (!p || p.lat == null) return;
      const ymd = el.dataset.ymd || placeVisitDate(p);
      const hm = el.dataset.hm || placeHour(p, ymd);
      const key = pwxKey(p, ymd);
      const compact = el.dataset.pwxMode === "compact";
      if (!pwx.cache[key]) {
        const elev = elevFor(p);
  ...
  function openPlace(id, ymd) {
    ...
    sheet.innerHTML = `
      <div class="panel glass">
        ...
        ${p.lat != null ? `<div class="pwx-slot" data-pwx="${esc(p.id)}" data-ymd="${esc(when)}" data-hm="${esc(hm)}"></div>` : ""}
        ${navPair(p)}
    ...
    return `<div class="timeline">${d.blocks.map((b) => {
      ...
      return `
        <article class="block ${b.hard ? "is-hard" : ""} ${done ? "done" : ""} ${b.status === "cancelled" ? "cancelled" : ""}">
          ...
          ${p && p.lat != null ? `<div class="pwx-slot" data-pwx="${p.id}" data-ymd="${d.date}" data-hm="${esc(b.time || "")}" data-pwx-mode="compact"></div>` : ""}
          ...
          <div class="row" style="margin-top:8px">
            ...
            ${p ? `<button class="pill" data-act="place" data-id="${p.id}" data-day="${d.date}">地圖</button>` : ""}
          </div>
        </article>`;
    }).join("")}</div>`;
  }
    ...
    return `${withBack("地點", "")}<section class="card glass">
      ...
      ${guide.map((p) => `
        <button class="menu" data-act="place" data-id="${p.id}">
          ...
          <div><b>${esc(p.name)}</b><div class="tiny">${esc(p.nameJa || "")}</div>
            ${p.lat != null ? `<div class="pwx-slot" data-pwx="${p.id}" data-ymd="${placeVisitDate(p)}" data-pwx-mode="compact"></div>` : ""}
          </div>
          <span class="go">›</span>
        </button>`).join("")}
    </section>`;
